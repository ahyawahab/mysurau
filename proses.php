...
class proses extends fb{
	...
	private function shutdown(){
		...
		if($this->dt=='s')
			...
			exec ("sudo reboot");
		$this->retError('Gagal...');//jika tidak restart/shutdown hantar yang ini
	}
	...
	private function getDatabase(){
		$file			= $this->file;
		//database belum ada? buat baru yang standard
		if (!file_exists($file)){
			$arr	= [
				'akses'	=> [
					'user'	=> 'admin',
				...
				'setting'	=> [
					'nama'		=> 'Hakimi System', // admin site
					'lokasi'	=> 'Ayer Keroh , Melaka',
					'latitude'	=> 3.137271,
					'longitude'	=> 101.374413,
					'timeZone'	=> 8,
					'dst'		=> '0'
				],
				...
				'prayTimesTune'	=> [	// jadual solat dlam waktu (minit)
					'fajr'		=> 0,
					'dhuhr'		=> 0,
					'asr'		=> 0,
					'maghrib'	=> 0,
				...
				'jumat'	=> [	// buat solat jumaat, boleh buat pilihan aktif atau tidak 
					'active'	=> true,
					'duration'	=> 60,	//durasi khutbah --> urutan : azan kutbah jumaat
					'text'		=> 'Harap diam di saat khotib membaca khutbah'
				],
				'tarawih'	=> [ // buat solat tarawih,boleh pilih aktif atau tidak
					'active'	=> true,
					'duration'	=> 180	//180 minit = 3 jam --> muncul display solat --> urutan : azan-iqomah-solat(isyak')-tarawih
				],
				'info'	=> [
					[
						'Aplikasi Display-Masjid',
						'Selamat datang di aplikasi Display Masjid, aplikasi baru saja diinstall dan masih belum ada data, masuk keadmin utnuk buat setting yang baru',
						'Display Masjid V.01',
						true	// active?
					],
					[
					...
					[
						...
						'Luruskanlah shaf-shaf kalian, karena lurusnya shaf adalah kesempurnaan solat',
				...
				'running_text' => [
					'Selamat datang di aplikasi Display Masjid V.01',
					'Silakan masuk ke menu admin untuk menukr data'
				]
			];
	...
	private function formSave(){
		...
		// unset($db['akses']);//proteksi biar user/password tidak di baca 
		// $this->data = $db;
			...
			foreach($_FILES as $file){
				if($file['size']>0){
					...
					if(!in_array($ext,$allowed_ext) ) {
						$this->retError($file['name']." tidak disokong\nExt yang dibolehkan : ".implode(", ",$allowed_ext));
					}
					else {
		...
		if(isset($_FILES)){
			...
			foreach($_FILES as $file){
				if($file['size']>0){
					$ext = pathinfo($file['name'], PATHINFO_EXTENSION);
					if(!in_array($ext,$allowed_ext) ) {
						$this->retError($file['name']." tidak support\nExt yang dibenarkan : ".implode(", ",$allowed_ext));
					}
					else {
						// tricky ==> kalau replace file logo.png ==> file di browser masih disimpan di cache ==> annoying	
						...
						// move_uploaded_file($file['tmp_name'], "display/img/logo.".$ext);//ganti logo masih simpan di cache browser
					}
				}
				$i++;
			}
		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<div class="box box-info">
				...
				<div class="box-body">
					...
					<div class="input">
						<small>
						- Ext file yang disupport :  <b>.jpg</b><br>
						- Ukuran maksimum <b>2Mb</b><br>
						- maksimum 5 file sekali upload<br>
						- Tips : Jika ukuran gambar > 2Mb, cara cepat kompress(padadkan) gambar ⇒ guna image magick
						</small>
					</div>
				</div>
				<div class="box-footer">
			...
			<div class="nav-tabs-custom">
				...
				<div class="tab-content">
				  ...
				  <div class="tab-pane" id="info" >
					Pengiraan waktu solat menggunakan library dari <a href="http://praytimes.org/" target="_blank">praytimes.org</a>, Untuk manual lebih detail boleh di cek pada site tersebut.<br>
					Library yang dipakai <b>PrayTimes Version 2.3</b> (versi terbaru pada waktu aplikasi ini dibangunkan)<br><br>
					Untuk memudahkan, setting parameter yang boleh di ganti hanya <b>fajar, zuhur, asar, maghrib, ishak</b> sesuaikan cara adjust pada display. Jika parameter tidak perlu diganti kosongkan saja (diisi default)
					
					<br><br>
					Contoh penggunaan untuk daearah klang ikut setting berikut :
					<pre>
latitude	= 3.092960 
longitude	= 101.570534
timeZone	= 8 (GMT +8)
fajr		= 20°
asr		= Standard (Shafii, Maliki, Jafari and Hanbali / shadow factor = 1)
isha		= 18°
					</pre>
					<small>Default aplikasi ini menggunakan setting <b>Selangor - klang - malaysia</b> dengan kaedah seperti diatas</small>
				  </div>
			...
			<div class="box box-warning">
				...
				<div class="box-body">
					...
					<div id="prayTimesAdjust" style="display:none">
						...
						<div class="form-group">
							<small>
								- Lihat manual parameter (diatas) untuk cara mengisi.<br>
								- Parameter <b>case sensitive</b> (contoh : <b>Standard</b> tidak sama dengan <b>standard</b>)<br>
								- Jika dibiarkan kosong maka akan diisi default.
							</small>
		...
		$set = [
			...
			'info'	=> '<b>DST</b> = Daylight Saving Time (Waktu Musim Panas)
						Waktu rasmi  (biasanya) satu jam lebih awal dari zon waktu standard malaysia )
			'
		];
		...
		$set = [
			'id'	=> 'prayTimesTune',
			'title'	=> 'Sesuaikan waktu solat',
			'info'	=> '- Untuk menyesuaikan waktu solat -60 sampai +60 minit.
						- Contoh penggunaan : jadual ditambahkan +2 minit untuk dihayati
			'
		];
	...
	private function pengaturan(){
		...
		<div class="box box-success ">
			...
			<div class="box-body">
				...
				<div class="input">
					<small>
					- Ext file yang disokong :  <b>.png</b><br>
					- Ukuran maksimum <b>2Mb</b><br>
					- Tips : jika logo keluar terlalu besar pada display, edit gambar pada image editor (contoh : photoshop) dan beri jarak kosong pada atas-bawah atau kiri-kanan gambar
					</small>
				</div>
			</div>
			<div class="box-footer">
		...
		$setSetting = [
			...
			'color'	=> 'box-success',
			'info'	=> '- Data ini tambahan (boleh dikosongkan)
			',
			'open'	=> false
		];
				...
				<div class="col-md-6">
				  ...
				  <div class="box box-widget widget-user-2">
					...
					<div class="widget-user-header bg-aqua-active">
					  ...
					  <h5 class="widget-user-desc">Saluran Media informasi untuk Surau|Masjid</h5>
					</div>
					<div class="box-footer no-padding" style="overflow:hidden">
					  <ul class="nav nav-stacked">
						...
						<li>
							<a class="row">
								<div class="col-xs-5" style="text-align:right">tarikh</div>
								<div class="col-xs-7"><span class="badge bg-aqua">November 2022</span></div>
							</a>
						</li>
						<li>
							<a class="row">
								<div class="col-xs-5" style="text-align:right">Program</div>
								<div class="col-xs-7">Nor Sairi Bin Ahmad Shahabudin</div>
							</a>
						</li>
						<li>
							<a class="row">
								<div class="col-xs-5" style="text-align:right">Display design</div>
								<div class="col-xs-7">Echobeam</div>
							</a>
						</li>
						<li>
							<a class="row">
								<div class="col-xs-5" style="text-align:right">License</div>
								<div class="col-xs-7">Sumber terbuka</div>
							</a>
						</li>
					  </ul>
					</div>
	...
	private function sistem(){
		...
		<div class="col-md-6 col-sm-12 col-xs-12">
			...
			<div class="box box-danger">
				<div class="box-header with-border">
					<h3 class="box-title">Reset Setting kepada asal</h3>
					<div class="box-tools pull-right">
						<button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
					</div>
				</div>
				<div class="box-body">
					- Semua data setting akan di reset kepada yang asal.<br>						
					- Logo dan wallpaper tidak berubah.<br>
					- Akses login kembali ke awal (user:admin, password:admin)<br>
					- Jika berjaya, akan masuk ke halaman login.
				</div>
				<div class="box-footer">
