<?php
	...
	if (!file_exists($file)){
		echo "<h1>masuk dalam admin terlebih dahulu</h1>";
		die;
	}
	...
	$info_timer			= $db['timer']['info'] 		* 1000;	//saat
	$wallpaper_timer	= $db['timer']['wallpaper'] * 1000;	
	$adzan_timer		= $db['timer']['adzan'] 	* 1000 * 60; //minit
	...
	//Logo
	//  trik ==> kalo replace file, di display logo yang lama masih simpan di cache ==>  jalan keluar ganti logo dan ganti nama file
	$dirLogo	= 'logo/';
...
<body>
	...
	<div id="count-down" class="full-screen" style="display:none">
		<div class="counter">
			...
			<div class="ii">00<span>MINIT</span></div>
			<div class="ss">00<span>SAAT</span></div>
		</div>
	</div>
	...
	<div id="right-counter" style="display:none">
		<div class="counter">
			...
			<div class="ii">25<span>MINIT</span></div>
			<div class="ss">45<span>SAAT</span></div>
		</div>
	</div>
			...
			<div class="item">
				...
				<?php 
					...
					if(PHP_OS=='Linux'){
						...
						//raspi 4
						$command="/sbin/ifconfig wlan0 | grep 'inet '| cut -d 't' -f2 | cut -d 'n' -f1 | awk '{ print $1}'";//raspi guna wlan0 untuk jadi hotspot
						$ip = trim(exec ($command));
					}
					if($db['akses']['pass']=='admin'){
						/**echo '<i class="fa fa-square-o" aria-hidden="true"></i>Masuk ke wifi (SSID: DisplayMasjid, password: 12345678)';
						...
						echo '<i class="fa fa-square-o" aria-hidden="true"></i> Sila menukar password admin untuk menghilangkan tulisan ini';
**/
					}
				?>
				</marquee>
    ...
    <script>
		...
		*/
		...
		?>
			...
			//Baris ini dan kebawah adalah untuk displit kan dan biarkan terpisah.....
		...
		var app	={
			...
			jadwalBesok		: {},
			timer			: false,
			// waitAdzanTimer	: false,	// Display countdown sebelum azan
			adzanTimer		: false,	// Display azan 
			countDownTimer	: false,	// Display countdown iqomah
			sholatTimer		: false,	// Display Solat
			khutbahTimer	: false,	// Display khutbah
			nextPrayCount	: 0,		// start next pray count-down
			// nextPrayTimer	: false,	// Display countdown ke solat seterusnya
			fajr	: '',
			dhuhr	: '',
			asr		: '',
			maghrib	: '',
			...
			showJadwal	: function(){
				...
				//+% minit baru berubah(contoh sekarang waktu zohur, dijadual selepas  minit baru berubar ke asar (akan diaktifkan))
				let jamDelay	= moment().subtract(5,'minutes');
				let jadwal	= '';
				let hari	= app.db.dayName[jamSekarang.format("dddd")];	//pastikan moment js pake standart inggeris (default) 
				...
				let jadwalPlusIcon	= '';
				//jika diatasa isyak pakai jadual yang semalam
				...
				$.each(app.db.prayName, function(k,v) {
					...
					else if	(k == 'fajr'	&& (jamDelay < app.fajr		|| jamDelay > app.isha))	css= 'active';//diatas isyak dan sebelum subuh (beza hari)
			...
			displaySchedule: function(){
				...
				$.each(app.db.prayName, function(k,v) {
					...
					let t			= moment(app[k]);//pakai variable baru t ==> jika ditulis let t	= app[k]; ==> jika di tambah / kurang, variable app[k] ikut berubah
					...
					if(waitAdzan == jadwal)				app.runRightCountDown(app[k],'Menuju '+v);	// CountDown sebelum adzan
						...
						if(moment().format('dddd')=='Friday' && app.db.jumat.active && k=='dhuhr'){
							...
							app.runFullCountDown(enIqomah,'IQAMAH',true);	// CountDown iqomah
					}
				});
			...
			showCountDownNextPray	: function(){
				...
				app.countDownTimer	= setInterval(function(){
					...
					app.nextPrayCount++;
					if (app.nextPrayCount >= 30) { // 30 saat show counter
			...
			showDisplayAdzan	: function(prayName){
				if (!app.adzanTimer){
					...
					},(app.db.timer.adzan * 60 * 1000)+1500);// to minit + 1.5 saat (sama dengan iqomah)
				}
			},
			showDisplayKhutbah	: function(){
				if (!app.khutbahTimer){
					...
					},app.db.jumat.duration * 60 * 1000);// to minit
				}
			},
